...
resource "aws_cloudwatch_metric_alarm" "httpcode_target_5xx_count" {
  alarm_name          = "${local.identifier}-alb-tg-${var.target_group_id}-high5XXCount"
...
resource "aws_cloudwatch_metric_alarm" "httpcode_lb_5xx_count" {
  alarm_name          = "${local.identifier}-alb-${var.load_balancer_id}-high5XXCount"
  comparison_operator = "GreaterThanThreshold"
  evaluation_periods  = var.evaluation_period
...
resource "aws_cloudwatch_metric_alarm" "target_response_time_average" {
  alarm_name          = "${local.identifier}-alb-tg-${var.target_group_id}-highResponseTime"
  comparison_operator = "GreaterThanThreshold"
  evaluation_periods  = var.evaluation_period
...
resource "aws_cloudwatch_metric_alarm" "unhealthy_hosts" {
  alarm_name          = "${local.identifier}-alb-tg-${var.target_group_id}-unhealthy-hosts"
  comparison_operator = "GreaterThanThreshold"
  evaluation_periods  = var.evaluation_period
